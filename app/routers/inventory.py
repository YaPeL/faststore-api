    ...
    try:
        with db.begin():
            ...
            if inventory:
                ...
                return {"message": f"Quantity incremented successfully. Current quantity: {inventory.quantity}"}
            else:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"An error occurred while incrementing the quantity.")
        ...
        with db.begin():
            ...
            if inventory:
                ...
                if inventory.quantity >= 0:
                    return {"message": f"Quantity decremented successfully. Current quantity: {inventory.quantity}"}
                else:
                    db.rollback()
                    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                        detail=f"Insufficient inventory: {inventory.quantity}")
            else:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"An error occurred while decreasing the quantity.")
